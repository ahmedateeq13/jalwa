...
function renderPlans(){
  ...
  plans.forEach(([number,amount,days]) => {
    ...
    card.innerHTML = `
      ...
      >
        ${savedStart
          ? "Approved â€” " + countdownText(savedStart,days)
...
function populateApproval(){
  ...
  plans.forEach(([number,amount,days]) => {
    ...
    option.textContent =
      `JALWA-${String(number).padStart(2,"0")} â€” Rs ${amount.toLocaleString()}${approved ? " â€” APPROVED" : ""}`;
...
function submitRequest(){
  ...
  if(requestType === "deposit"){
    ...
    toast(
      "Deposit request submitted â€” PENDING DEMO."
    );
    ...
    toast(
      "Withdrawal request submitted â€” PENDING DEMO."
    );
...
function openHistory(){
  ...
  toast(
    `${last.type}: ${last.status} â€” Rs ${Number(last.amount).toLocaleString()}`
  );
